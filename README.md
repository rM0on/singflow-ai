...
Health routes can run with the local API process. Database-backed business routes require PostgreSQL, Alembic migration, and demo bootstrap. Phase 2G verified those backend routes in local Docker; the frontend remains a portfolio-grade static prototype until a later frontend-backend integration phase.
...
This is local Docker verification, not a cloud release. The frontend still needs a future integration pass before it calls the backend APIs directly.
